defmodule SmokeTest do
  ...
  def init(port) do
    ...
    case :gen_tcp.listen(port, listen_options) do
      {:ok, listen_socket} ->
        Logger.info("Starting SmokeTest on port #{port}")
  ...
  def handle_continue(:accept, %__MODULE__{} = state) do
    case :gen_tcp.accept(state.listen_socket) do
      ...
      {:error, reason} ->
        Logger.error("[SmokeTest] Unable to accept connection #{inspect(reason)}")
        {:stop, reason, state}
    end
  end
  ...
  defp handle_connection(socket) do
    case receive_until_closed(socket, _buffer = "") do
      ...
      {:error, reason} ->
        Logger.error("[SmokeTest] Failed to receive data #{inspect(reason)}")
    end
