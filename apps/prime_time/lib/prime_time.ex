defmodule PrimeTime do
  ...
  def init(port) do
    ...
    case :gen_tcp.listen(port, listen_options) do
      {:ok, listen_socket} ->
        Logger.info("Starting PrimeTime on port #{port}")
  ...
  def handle_continue(:accept, %__MODULE__{} = state) do
    case :gen_tcp.accept(state.listen_socket) do
      ...
      {:error, reason} ->
        Logger.error("[PrimeTime] Unable to accept connection #{inspect(reason)}")
        {:stop, reason, state}
    end
  end
  ...
  defp respond_to_lines_until_closed(socket, task_id) do
    with {:ok, line} <- :gen_tcp.recv(socket, _bytes_to_read = 0, _timeout_millis = 30_000),
      ...
      Logger.info("[PrimeTime] task_id=#{task_id} : response : #{number} prime? #{prime?}")
      send_response(socket, prime?)
      respond_to_lines_until_closed(socket, task_id)
    else
      # Jason decoded, but the decoded request is invalid
      {:ok, decoded} ->
        Logger.error("[PrimeTime] task_id=#{task_id} : invalid request : #{inspect(decoded)}")
        send_malformed(socket)

      # Jason could not decode json
      {:error, error = %Jason.DecodeError{}} ->
        Logger.error("[PrimeTime] task_id=#{task_id} : #{inspect(error)}")
        send_malformed(socket)
